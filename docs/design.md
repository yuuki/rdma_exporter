...
- **Metrics**:
  ...
  - RoCEv2 PFC and ethtool hardware counters (`rdma_roce_pfc_*`, `rdma_netdev_*`, `rdma_pcie_*`, `rdma_phy_*`, including IEEE 802.3x global pause, pause storm, and vport RDMA) behind `--collector.ethtool` (default on).
  - Optional hardware counters from RDMA netlink (`rdma_optional_counter_enabled`, `rdma_cc_*_total`, `rdma_optional_{rx,tx}_{bytes,packets}_total`) behind `--collector.optional-counters` (default on). The exporter never enables counters.
  - Live auto-type QP counters (`rdma_qp_*`) behind `--collector.qp-counters` (default off). The exporter never binds QPs or enables auto mode. These are not a partition of sysfs port totals.
...
- **Flags**:
  ...
  - `--collector.ethtool` (default true; PFC and hardware ethtool families together; disable with `--no-collector.ethtool`)
  - `--collector.optional-counters` (default true; NETLINK_RDMA; mlx5 `cc_*` and `rdma_{rx,tx}_{bytes,packets}`; does not enable counters in the kernel)
  - `--collector.qp-counters` (default false; separate NETLINK_RDMA socket; does not bind QPs or enable auto mode; dump only when port mode is auto type-only)
...
- A custom counter `rdma_scrape_errors_total` tracks failures fetching sysfs data. `rdma_roce_pfc_scrape_errors_total` and `rdma_netdev_scrape_errors_total` track ethtool failures for their respective families (both increment on the same cached `Stats()` failure). `rdma_optional_counter_scrape_errors_total` tracks NETLINK_RDMA optional-counter failures. `rdma_qp_scrape_errors_total` tracks QP dump/mode failures, including receive-budget overflow. `rdma_scrape_collector_success{collector}` reports whether each enabled collector was usable on this scrape.
