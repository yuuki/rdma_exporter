...
| Congestion: IEEE 802.3x global pause / storm    | `rdma_netdev_global_pause_duration_total` via `rate()/1e6`, `rdma_netdev_pause_storm_events_total`                                            | Not on the bundled JSON yet. Requires `--enable-netdev-hw-metrics`. Distinct from PFC; keys exist only in global pause mode. |
