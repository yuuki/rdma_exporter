...
| Congestion: pause storm [events/s] / port       | `rdma_netdev_pause_storm_events_total` via `rate()`                                                                                           | warning = watermark; error = timeout and pause TX disabled. Requires `--enable-netdev-hw-metrics`.             |
| Congestion: IEEE 802.3x pause frames [frames/s] | `rdma_netdev_global_pause_frames_total` via `rate()`                                                                                          | Observation only. direction=rx received a pause request; tx transmitted one. Requires `--enable-netdev-hw-metrics`. |
